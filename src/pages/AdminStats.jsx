  ...
  useEffect(() => {
    const load = async () => {
      const [{ count: totalUsers }, { data: profiles }, { data: deposits }, { data: txns }] = await Promise.all([
        ...
        supabase.from("profiles").select("id, full_name, email, balance"),
      ...
      setStats({ totalUsers: totalUsers || 0, totalVolume: confirmed.reduce((a, d) => a + (d.amount || 0), 0), totalDeposits: confirmed.length, totalPending: deposits?.filter(d => d.status === "pending").length || 0 });
      setLeaderboard((profiles || []).sort((a, b) => (b.balance || 0) - (a.balance || 0)).slice(0, 10));
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: "24px" }}>
      <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: "20px" }}>
        {[
          ...
          { label: "Confirmed Volume", value: `$${stats.totalVolume.toFixed(2)}`,         color: "#22c55e" },
        ...
        <div style={{ background: "white", borderRadius: "16px", border: "1px solid #e2e8f0", overflow: "hidden" }}>
          ...
          {leaderboard.map((u, i) => (
            <div key={u.id} style={{ padding: "14px 24px", borderBottom: "1px solid #f8fafc", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
              ...
              <span style={{ fontWeight: "800", fontSize: "15px", color: "#22c55e" }}>${(u.balance || 0).toLocaleString("en-US", { minimumFractionDigits: 2 })}</span>
            </div>
          ))}
        </div>
            ...
            {transactions.length === 0 ? (
              ...
              <div key={t.id} style={{ padding: "14px 24px", borderBottom: "1px solid #f8fafc", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                <div style={{ display: "flex", alignItems: "center", gap: "10px" }}>
                  <div style={{ width: 32, height: 32, borderRadius: "8px", background: t.type === "buy" ? "#f0fdf4" : t.type === "sell" ? "#fef2f2" : "#eff6ff", display: "flex", alignItems: "center", justifyContent: "center" }}>
                    {t.type === "buy" ? <TrendingUp size={14} color="#22c55e" /> : t.type === "sell" ? <TrendingDown size={14} color="#ef4444" /> : <span style={{ fontSize: "12px" }}>💰</span>}
                  </div>
                  <div>
                    <p style={{ margin: 0, fontWeight: "600", fontSize: "13px", color: "#0f172a", textTransform: "capitalize" }}>{t.type} {t.symbol || ""}</p>
                    <p style={{ margin: 0, fontSize: "12px", color: "#94a3b8" }}>{t.profiles?.full_name || t.profiles?.email || "—"}</p>
                  </div>
                </div>
                <div style={{ textAlign: "right" }}>
                  <p style={{ margin: 0, fontWeight: "700", fontSize: "14px", color: t.type === "sell" ? "#ef4444" : "#22c55e" }}>{t.type === "sell" ? "-" : "+"}${t.amount_usd?.toFixed(2)}</p>
